	...
	if ( laserBeamComponent ) then
		...
		if ( currentTarget ~= INVALID_ID ) then		
			...
			elseif ( currentTarget == self.lastTarget ) then
				...
				self.scanningTime = self.scanningTime + ( 1.0 / 3.0 )
				self.factor =  self.scanningTime / self.maxScanTime
				EffectService:SetParticleEmmissionUniform( self.effect, self.factor )

-------------------------------------------------------------------------------------------------
--- Scanner Instant Mod
--- Original: if ( self.scanningTime >= self.maxScanTime ) then

				if ( self.scanningTime >= 0 ) then 
-------------------------------------------------------------------------------------------------

-------------------------------------------------------------------------------------------------
--- 10x Specimen Mod
--- Original: ItemService:ScanEntity( currentTarget, self.owner )
